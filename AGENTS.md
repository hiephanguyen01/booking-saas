...
1. **Exactly two kinds of test exist; anything else is forbidden.**
   **(a) One use case = one unit test**, `*.use-case.spec.ts` beside it — every one of them has one, and
...
- [`docs/decisions/`](./docs/decisions/) — ADRs (opaque sessions, RLS, outbox, migrations, no tests, no services, listing edit revisions, legal documents, limited tests, L1 caching & security hardening)
