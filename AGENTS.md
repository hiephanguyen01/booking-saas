# AGENTS.md — Bookify: Booking SaaS + Marketplace (Turborepo + NestJS + React Router 7)
...
> non-Codex tools (Codex, Cursor, Gemini CLI, …) here. The skill invocations in Section 1 apply
...
1. [Codex Skills & Tools to Use](#1-Codex-skills--tools-to-use)
...
## 1. Codex Skills & Tools to Use

When working on this project, Codex **must** invoke the appropriate skills listed below.
Each skill provides specialized knowledge and patterns — never guess at conventions when a skill covers the topic.

**All project skills are vendored in `.Codex/skills/` and checked into this repo** — every
clone has them available without any per-machine installation. (`/security-review`, `/review`,
`/run`, `/verify`, and `/simplify` are Codex built-ins and need no install.)
...
_End of AGENTS.md — keep this file updated as the project evolves._
