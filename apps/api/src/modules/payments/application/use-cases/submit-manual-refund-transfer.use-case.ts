...
export class SubmitManualRefundTransferUseCase {
  ...
  async execute(
    tenantId: string,
    operationId: string,
    input: SubmitManualRefundTransferInput,
    actorUserId: string,
  ) {
    const outcome = await this.tenantDb.forTenant(tenantId, async (tx) => {
      ...
      if (!isManualRefundEvidenceKey(tenantId, operationId, input.evidenceObjectKey))
      ...
      const inspection = await this.storage.inspectPrivateFile({
        key: upload.objectKey,
        allowedContentTypes: ALLOWED,
        maxSizeBytes: MAX_MANUAL_REFUND_EVIDENCE_SIZE_BYTES,
      });
      if (
        !inspection.valid ||
      ...
      if (!(await this.evidence.claimUpload(tx, tenantId, upload.id, now)))
        throw new ManualRefundEvidenceUploadInvalid();
      const entity = toManualRefundOperation({
        ...current,
        evidenceObjectKey: upload.objectKey,
        evidenceContentType: upload.contentType,
        evidenceSizeBytes: upload.sizeBytes,
        evidenceSha256: upload.checksum,
        evidenceVerifiedAt: now,
        transferReference: input.reference,
      });
      entity.submitTransfer(actorUserId);
      ...
      await this.outbox?.emit(tx, {
        tenantId,
        eventType: 'manual_refund.transfer_submitted',
        payload: { operationId, refundBatchId: current.refundBatchId },
      });
      await this.audit.write(tx, {
        tenantId,
        actorUserId,
        action: 'manual_refund.transfer_submitted',
        entityType: 'manual_refund_operation',
        entityId: operationId,
        data: { evidencePresent: true },
      });
