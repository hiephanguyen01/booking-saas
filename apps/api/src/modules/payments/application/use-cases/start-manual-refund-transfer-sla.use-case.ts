...
export class StartManualRefundTransferSlaUseCase {
  ...
  async execute(tenantId: string, operationId: string, slaHours: number): Promise<boolean> {
    return this.tenantDb.forTenant(tenantId, async (tx) => {
      ...
      if (!current || current.status !== 'ready_for_transfer' || current.transferDueAt)
        return false;
      if (!current.readyAt || !Number.isInteger(slaHours) || slaHours < 1 || slaHours > 720)
        return false;
      const updated = await this.operations.casUpdate(
        ...
        current.status,
        current.version,
        { transferDueAt: new Date(current.readyAt.getTime() + slaHours * 60 * 60 * 1000) },
      );
