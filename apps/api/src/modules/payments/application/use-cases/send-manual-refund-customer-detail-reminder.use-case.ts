...
export class SendManualRefundCustomerDetailReminderUseCase {
  ...
  async execute(tenantId: string, operationId: string, hours: 24 | 48): Promise<boolean> {
    return this.tenantDb.forTenant(tenantId, async (tx) => {
      ...
      const marker =
        hours === 24 ? current.customerDetailReminder24At : current.customerDetailReminder48At;
      ...
      const updated = await this.operations.casUpdate(
        ...
        current.status,
        current.version,
        hours === 24 ? { customerDetailReminder24At: now } : { customerDetailReminder48At: now },
      );
      if (!updated) throw new ManualRefundConcurrentUpdate();
      await this.outbox.emit(tx, {
        tenantId,
        eventType: 'manual_refund.customer_details_reminder',
        payload: { operationId, refundBatchId: current.refundBatchId, hours },
      });
      return true;
    });
  }
