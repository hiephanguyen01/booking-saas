...
export class ReopenManualRefundDestinationUseCase {
  ...
  async execute(
    ...
    const invalidatedKeys = await this.tenantDb.forTenant(tenantId, async (tx) => {
      ...
      entity.reopen({ actorUserId, reason: input.reason, occurredAt: now });
      ...
      await this.audit.write(tx, {
        tenantId,
        actorUserId,
        action: 'manual_refund.destination_reopened',
    ...
    await Promise.all(
      invalidatedKeys.keys.map(async (key) => {
        try {
