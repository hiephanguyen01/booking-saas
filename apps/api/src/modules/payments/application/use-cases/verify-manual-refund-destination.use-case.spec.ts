...
describe('VerifyManualRefundDestinationUseCase', () => {
  it('CAS-verifies an unsupported lookup and audits without PII', async () => {
    ...
    expect(patches[0]).toMatchObject({
      ...
      verifiedByUserId: MANUAL_REFUND_CHECKER_ID,
      readyAt: MANUAL_REFUND_NOW,
    });
  ...
  it('cannot override a lookup mismatch', async () => {
    ...
    await expect(
      useCase.execute(
        MANUAL_REFUND_TENANT_ID,
        MANUAL_REFUND_OPERATION_ID,
        { expectedVersion: 3, outcome: 'matched', note: 'Override mismatch' },
        MANUAL_REFUND_CHECKER_ID,
      ),
    ).rejects.toBeInstanceOf(ManualRefundAccountMismatch);
  });
