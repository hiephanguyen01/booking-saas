...
describe('CreateManualRefundEvidenceUploadUseCase', () => {
  it('registers a private write-once receipt grant scoped to the claimed operation', async () => {
    ...
    const grant = {
      uploadUrl: 'https://private.example/put',
      key: `manual-refund-evidence/${MANUAL_REFUND_TENANT_ID}/${MANUAL_REFUND_OPERATION_ID}/receipt.pdf`,
      expiresInSec: 300,
    };
    const useCase = new CreateManualRefundEvidenceUploadUseCase(
      fakePort<IManualRefundOperationRepository>({
        ...
        findById: () => Promise.resolve(manualRefundOperation()),
      }),
      fakePort<IManualRefundEvidenceRepository>({
        createUpload: (_tx, _tenant, data) => {
          records.push(data);
      ...
      fakeTenantDb({ now: new Date('2026-09-04T13:00:00Z') }).service,
    );
    await expect(
      useCase.execute(
        MANUAL_REFUND_TENANT_ID,
    ...
    expect(grants[0]).toMatchObject({
      keyPrefix: `manual-refund-evidence/${MANUAL_REFUND_TENANT_ID}/${MANUAL_REFUND_OPERATION_ID}`,
      contentType: 'application/pdf',
    ...
    expect(records[0]).toMatchObject({
      ...
      objectKey: grant.key,
      checksum: 'b'.repeat(64),
      expiresAt: new Date('2026-09-05T13:00:00Z'),
    });
  });
  ...
  it('rejects a stale operation version with a named concurrency conflict', async () => {
    const useCase = new CreateManualRefundEvidenceUploadUseCase(
      fakePort<IManualRefundOperationRepository>({
        getWorkflowState: () => Promise.resolve({ enabled: true, paused: false }),
        findById: () => Promise.resolve(manualRefundOperation({ version: 4 })),
      }),
      fakePort<IManualRefundEvidenceRepository>({}),
      fakePort<StoragePort>({
