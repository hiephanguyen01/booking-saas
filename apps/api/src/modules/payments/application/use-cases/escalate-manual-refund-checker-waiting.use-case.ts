...
export class EscalateManualRefundCheckerWaitingUseCase {
  ...
  async execute(tenantId: string, operationId: string, waitingHours = 24): Promise<boolean> {
    return this.tenantDb.forTenant(tenantId, async (tx) => {
      ...
      if (
        current.status !== 'transfer_submitted' ||
        !current.transferSubmittedAt ||
        current.checkerEscalatedAt
      )
        return false;
      const now = await this.tenantDb.databaseNow(tx);
      if (now.getTime() < current.transferSubmittedAt.getTime() + waitingHours * 60 * 60 * 1000)
        return false;
      const updated = await this.operations.casUpdate(
        ...
        current.version,
        {
          checkerWaitingAt: current.checkerWaitingAt ?? current.transferSubmittedAt,
          checkerEscalatedAt: now,
        },
      );
      if (!updated) throw new ManualRefundConcurrentUpdate();
      await this.outbox.emit(tx, {
        tenantId,
        eventType: 'manual_refund.checker_escalated',
