...
describe('ReassignManualRefundUseCase', () => {
  it('CAS-reassigns a claimed operation with actor and reason', async () => {
    ...
    const useCase = new ReassignManualRefundUseCase(fakePort<IManualRefundOperationRepository>({ getWorkflowState: () => Promise.resolve({ enabled: true, paused: false }), findById: () => Promise.resolve(current), casUpdate: (_tx, _tenant, _id, _status, _version, patch) => { patches.push(patch); return Promise.resolve({ ...current, ...patch, version: 4 }); } }), fakePort<IAuditWriter>({ write: () => Promise.resolve() }), fakeTenantDb({ now: MANUAL_REFUND_NOW }).service);
  ...
  it('rejects with MANUAL_REFUND_WORKFLOW_PAUSED when workflow is paused', async () => {
    ...
    const useCase = new ReassignManualRefundUseCase(
    ...
    await expect(
      useCase.execute(
        ...
        { expectedVersion: 3, makerUserId: MANUAL_REFUND_CHECKER_ID, reason: 'Shift handover' },
        'actor-1',
      ),
