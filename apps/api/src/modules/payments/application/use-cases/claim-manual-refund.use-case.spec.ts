...
describe('ClaimManualRefundUseCase', () => {
  it('claims by expected status/version CAS and records the maker', async () => {
    ...
    const useCase = new ClaimManualRefundUseCase(fakePort<IManualRefundOperationRepository>({ getWorkflowState: () => Promise.resolve({ enabled: true, paused: false }), findById: () => Promise.resolve(current), casUpdate: (...args) => { seen.push(args.slice(2)); return Promise.resolve({ ...current, makerUserId: MANUAL_REFUND_MAKER_ID, claimedAt: MANUAL_REFUND_NOW, version: 4 }); } }), fakePort<IAuditWriter>({ write: () => Promise.resolve() }), tenantDb.service);
