...
describe('EscalateManualRefundCheckerWaitingUseCase', () => {
  it('emits one escalation after the checker waiting threshold', async () => {
    ...
    const current = manualRefundOperation({
      ...
      transferSubmittedAt: new Date('2026-09-03T12:00:00Z'),
      checkerEscalatedAt: null,
    });
    const operations = fakePort<IManualRefundOperationRepository>({
      ...
      casUpdate: (_tx, _tenant, _id, _status, _version, patch) =>
        Promise.resolve({ ...current, ...patch, version: current.version + 1 }),
    });
    const outbox = new OutboxService();
    vi.spyOn(outbox, 'emit').mockImplementation(async (_tx, event) => {
      events.push(event);
    });
    const useCase = new EscalateManualRefundCheckerWaitingUseCase(
      operations,
      outbox,
    ...
    expect(events[0]).toMatchObject({
      ...
      payload: { operationId: MANUAL_REFUND_OPERATION_ID },
    });
  });
});
