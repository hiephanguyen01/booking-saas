...
describe('ReopenManualRefundDestinationUseCase', () => {
  it('invalidates the maker, transfer draft, and evidence through CAS', async () => {
    ...
    const useCase = new ReopenManualRefundDestinationUseCase(fakePort<IManualRefundOperationRepository>({ getWorkflowState: () => Promise.resolve({ enabled: true, paused: false }), findById: () => Promise.resolve(current), casUpdate: (_tx, _tenant, _id, _status, _version, patch) => { patches.push(patch); return Promise.resolve({ ...current, ...patch, version: 4 }); } }), fakePort<IManualRefundEvidenceRepository>({ invalidateUploads: () => Promise.resolve(['private/x.pdf']) }), fakePort<StoragePort>({ quarantinePrivateObject: (key) => { invalidated.push(key); return Promise.resolve(); } }), fakePort<IAuditWriter>({ write: () => Promise.resolve() }), fakeTenantDb({ now: MANUAL_REFUND_NOW }).service);
  ...
  it('returns the committed reopened projection when object quarantine is temporarily unavailable', async () => {
    ...
    const useCase = new ReopenManualRefundDestinationUseCase(
      fakePort<IManualRefundOperationRepository>({ getWorkflowState: () => Promise.resolve({ enabled: true, paused: false }), findById: () => Promise.resolve(current), casUpdate: (_tx, _tenant, _id, _status, _version, patch) => Promise.resolve({ ...current, ...patch, version: 4 }) }),
  ...
  it('rejects with MANUAL_REFUND_WORKFLOW_PAUSED when workflow is paused', async () => {
    ...
    await expect(
      useCase.execute(
        ...
        { expectedVersion: 3, reason: 'Customer must correct account' },
