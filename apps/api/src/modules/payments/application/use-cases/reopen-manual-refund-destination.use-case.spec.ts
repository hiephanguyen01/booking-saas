...
describe('ReopenManualRefundDestinationUseCase', () => {
  it('invalidates the maker, transfer draft, and evidence through CAS', async () => {
    ...
    const current = manualRefundOperation({
      ...
      transferReference: 'VCB-1',
      evidenceObjectKey: 'private/x.pdf',
    ...
    const useCase = new ReopenManualRefundDestinationUseCase(
      fakePort<IManualRefundOperationRepository>({
        ...
        casUpdate: (_tx, _tenant, _id, _status, _version, patch) => {
          patches.push(patch);
          return Promise.resolve({ ...current, ...patch, version: 4 });
        },
      }),
      fakePort<IManualRefundEvidenceRepository>({
        invalidateUploads: () => Promise.resolve(['private/x.pdf']),
      }),
    ...
    expect(patches[0]).toMatchObject({
      ...
      reopenedByUserId: MANUAL_REFUND_CHECKER_ID,
      reopenReason: 'Customer must correct account',
    });
  ...
  it('returns the committed reopened projection when object quarantine is temporarily unavailable', async () => {
    ...
    const useCase = new ReopenManualRefundDestinationUseCase(
      fakePort<IManualRefundOperationRepository>({
        ...
        findById: () => Promise.resolve(current),
        casUpdate: (_tx, _tenant, _id, _status, _version, patch) =>
          Promise.resolve({ ...current, ...patch, version: 4 }),
      }),
      fakePort<IManualRefundEvidenceRepository>({
        invalidateUploads: () => Promise.resolve(['private/x.pdf']),
      }),
      fakePort<StoragePort>({
        quarantinePrivateObject: () => Promise.reject(new Error('storage unavailable')),
      }),
    ...
    await expect(
      useCase.execute(
        MANUAL_REFUND_TENANT_ID,
        MANUAL_REFUND_OPERATION_ID,
        { expectedVersion: 3, reason: 'Retry destination' },
        MANUAL_REFUND_CHECKER_ID,
      ),
    ).resolves.toMatchObject({ status: 'awaiting_details', version: 4 });
    expect(audits).toContainEqual(
      expect.objectContaining({
        action: 'manual_refund.evidence_quarantine_failed',
        data: { failedObjectCount: 1 },
      }),
    );
