...
export class DispatchNotificationService {
  ...
  async dispatchBookingEvent(tenantId: string, eventType: string, payload: BookingEventPayload): Promise<void> {
    const plan = planForEvent(eventType, payload);
    if (plan.length === 0) return;
    const ctx = await this.tenantDb.forTenant(tenantId, (tx) => this.reader.loadBookingContext(tx, payload.bookingId));
    if (!ctx) return;
    ...
    for (const item of plan) {
      ...
      for (const recipient of recipients) {
        ...
        await this.deliver(tenantId, eventType, recipient, item, this.bookingData(ctx, recipient, payload), dedupeKey, ctx.bookingId);
      }
    }
  }

  /** listing.published / listing.hidden → the owning partner's members. */
  async dispatchListingEvent(tenantId: string, eventType: string, payload: { listingId: string; reason?: string }): Promise<void> {
    const plan = planForEvent(eventType, {});
    if (plan.length === 0) return;
    const ctx = await this.tenantDb.forTenant(tenantId, (tx) => this.reader.loadListingContext(tx, payload.listingId));
    if (!ctx) return;
    for (const item of plan) {
      for (const recipient of ctx.partnerRecipients) {
        const dedupeKey = `${eventType}:${payload.listingId}:${item.templateId}:${recipient.userId}`;
        const data: TemplateData = { tenantName: ctx.tenantName, recipientName: recipient.name, listingTitle: ctx.listingTitle, reason: payload.reason };
  ...
  async dispatchPartnerEvent(tenantId: string, eventType: string, payload: { partnerId: string }): Promise<void> {
    const plan = planForEvent(eventType, {});
    if (plan.length === 0) return;
    const ctx = await this.tenantDb.forTenant(tenantId, (tx) => this.reader.loadPartnerContext(tx, payload.partnerId));
    if (!ctx) return;
    for (const item of plan) {
      for (const recipient of ctx.recipients) {
        const dedupeKey = `${eventType}:${payload.partnerId}:${item.templateId}:${recipient.userId}`;
        const data: TemplateData = { tenantName: ctx.tenantName, recipientName: recipient.name, partnerName: ctx.partnerName };
        await this.deliver(tenantId, eventType, recipient, item, data, dedupeKey, null);
      }
    }
  }
  ...
  async dispatchReminder(tenantId: string, bookingId: string): Promise<void> {
    const ctx = await this.tenantDb.forTenant(tenantId, (tx) => this.reader.loadBookingContext(tx, bookingId));
    if (!ctx?.customer) return;
    const item: NotificationPlanItem = { audience: 'customer', templateId: 'booking_reminder_customer' };
    const dedupeKey = `booking.reminder:${bookingId}:${ctx.customer.userId}`;
    await this.deliver(tenantId, 'booking.reminder', ctx.customer, item, this.bookingData(ctx, ctx.customer, {}), dedupeKey, bookingId);
  }

  private audienceRecipients(item: NotificationPlanItem, ctx: BookingNotificationContext): NotificationRecipient[] {
  ...
  private async deliver(
    ...
    try {
      await this.email.send({ to: recipient.email, subject: content.subject, text: content.text, html: content.html });
