...
import { DeleteTenantCancellationPolicyUseCase } from './delete-tenant-cancellation-policy.use-case';
...
function harness(existing: unknown, inUse = 0) {
  ...
  return {
    useCase: new DeleteTenantCancellationPolicyUseCase(
...
describe('DeleteTenantCancellationPolicyUseCase', () => {
  it('answers not-found for a policy this tenant does not have', async () => {
    const { useCase, deleted } = harness(null);

    await expect(useCase.execute(TENANT_ID, POLICY_ID)).rejects.toBeInstanceOf(
      CancellationPolicyNotFound,
    );
  ...
  it('refuses a policy it does not own', async () => {
    const { useCase, deleted } = harness(policy(PARTNER_ID));
  ...
  it('refuses while a listing still points at it', async () => {
    ...
    // nothing to snapshot.
    const { useCase, deleted } = harness(policy(null), 2);
  ...
  it('deletes an unused policy it owns', async () => {
    const { useCase, tenantDb, deleted } = harness(policy(null), 0);

    await useCase.execute(TENANT_ID, POLICY_ID);
