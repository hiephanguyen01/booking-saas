...
import { CalendarClock, Copy, EyeOff, Info, Layers3, Lock, Pencil, Plus, RotateCcw, Send, Trash2 } from 'lucide-react';
import type { ListingGroupDetailResponse, ListingResponse, ListingTypeResponse, PublishStatus } from '@booking/contracts';
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  if (['submit', 'reopen', 'delete-group', 'delete-child', 'duplicate-child'].includes(intent) && !canWrite) {
  ...
  if (intent === 'hide' || intent === 'republish') {
    ...
    return res.ok ? data({ ok: true, error: null }) : data({ ok: false, error: res.error ?? 'Thao tác không thành công.' }, { status: 400 });
  }
  if (intent === 'reopen') {
    ...
    return res.ok ? data({ ok: true, error: null }) : data({ ok: false, error: res.error ?? 'Không thể chuyển bài đăng về bản nháp.' }, { status: 400 });
  }
  if (intent === 'delete-group') {
    const res = await apiDelete(`/partner/listing-groups/${params.groupId}`, auth);
    if (!res.ok) return data({ ok: false, error: res.error ?? 'Xóa bài đăng không thành công.' }, { status: 400 });
...
const STATUS_META: Record<PublishStatus, { label: string; title: string; description: string }> = {
  ...
  published: {
    ...
    description: 'Bạn vẫn có thể quản lý giờ hoạt động. Hãy ẩn bài đăng trước khi sửa nội dung hạng mục.',
  },
  archived: {
    label: 'Đã ẩn',
...
export default function ListingGroupWorkspace({ loaderData, actionData }: Route.ComponentProps) {
  ...
  const columns: DataTableColumn<ListingResponse>[] = [
    ...
    { header: 'Thao tác', className: 'text-right', headClassName: 'text-right', cell: (listing) => <GroupedListingActions groupId={group.id} listing={listing} itemLabel={itemLabel} canEdit={canEditItems} canManageHours={canAvailability && usesOpeningHours(listing)} /> },
  ];

  return <div className="flex flex-col gap-6">
    <PageHeader title={group.title} description={`${loaderData.listingType?.name ?? 'Bài đăng'} · ${group.listingCount} ${itemLabel}`} actions={<><Badge variant="outline">{statusMeta.label}</Badge>{canWrite && ['draft', 'archived'].includes(group.status) && !adminLocked ? <Button asChild variant="outline" size="sm"><Link to={`/partner/listing-groups/${group.id}/edit`}><Pencil data-icon="inline-start" /> Sửa thông tin chung</Link></Button> : null}</>} />
    ...
    <Alert>
      ...
      <AlertDescription>
        <p>{adminLocked ? 'Bạn có thể xem nội dung và quản lý giờ hoạt động, nhưng chỉ quản trị viên mới có thể bỏ ẩn bài đăng.' : statusMeta.description}</p>
        <GroupLifecycleActions group={group} canWrite={canWrite} canPublish={canPublish} adminLocked={adminLocked} />
      </AlertDescription>
    </Alert>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
      <Card><CardHeader className="flex-row flex-wrap items-center justify-between gap-3"><div><CardTitle className="capitalize">{itemLabel} & giá</CardTitle><CardDescription>Những lựa chọn khách hàng có thể đặt trong bài đăng này.</CardDescription></div>{canEditItems ? <Button asChild size="sm"><Link to={`/partner/listing-groups/${group.id}/listings/new`}><Plus data-icon="inline-start" /> Thêm {itemLabel}</Link></Button> : null}</CardHeader><CardContent>{group.listings.length === 0 ? <Empty><EmptyHeader><EmptyMedia variant="icon"><Layers3 /></EmptyMedia><EmptyTitle>Chưa có {itemLabel} nào</EmptyTitle><EmptyDescription>Thêm ít nhất một {itemLabel} mà khách hàng có thể chọn và đặt.</EmptyDescription></EmptyHeader>{canEditItems ? <EmptyContent><Button asChild><Link to={`/partner/listing-groups/${group.id}/listings/new`}><Plus data-icon="inline-start" /> Thêm {itemLabel}</Link></Button></EmptyContent> : null}</Empty> : <><div className="hidden md:block"><DataTable columns={columns} data={group.listings} getRowKey={(listing) => listing.id} /></div><div className="grid gap-3 md:hidden">{group.listings.map((listing) => <GroupedListingCard key={listing.id} groupId={group.id} listing={listing} itemLabel={itemLabel} canEdit={canEditItems} canManageHours={canAvailability && usesOpeningHours(listing)} />)}</div></>}</CardContent></Card>
...
function SubmitGroupButton({ disabled }: { disabled: boolean }) {
  ...
  return <div className="flex flex-col items-start gap-1"><Button disabled={disabled || fetcher.state !== 'idle'} onClick={() => fetcher.submit({ intent: 'submit' }, { method: 'post' })}><Send data-icon="inline-start" /> Gửi duyệt</Button>{fetcher.data?.error ? <p className="text-xs text-destructive" role="alert">{fetcher.data.error}</p> : null}</div>;
}

function GroupLifecycleActions({ group, canWrite, canPublish, adminLocked }: { group: ListingGroupDetailResponse; canWrite: boolean; canPublish: boolean; adminLocked: boolean }) {
  const fetcher = useFetcher<typeof action>();
  const busy = fetcher.state !== 'idle';
  const submit = (intent: string) => fetcher.submit({ intent }, { method: 'post' });

  return <div className="mt-2 flex flex-wrap gap-2">
    {group.status === 'published' && canPublish ? <Button size="sm" variant="outline" disabled={busy} onClick={() => submit('hide')}><EyeOff /> Ẩn để chỉnh sửa</Button> : null}
    {group.status === 'archived' && canWrite && !adminLocked ? <Button size="sm" disabled={busy} onClick={() => submit('reopen')}><Pencil /> Chuyển về bản nháp</Button> : null}
    {group.status === 'archived' && canPublish && !adminLocked ? <Button size="sm" variant="outline" disabled={busy} onClick={() => submit('republish')}><RotateCcw /> Đăng lại</Button> : null}
    {['draft', 'archived'].includes(group.status) && canWrite && group.listingCount === 0 && !adminLocked ? <AlertDialog><AlertDialogTrigger asChild><Button size="sm" variant="ghost" disabled={busy} className="text-destructive hover:text-destructive"><Trash2 /> Xóa bài đăng</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xóa bài đăng?</AlertDialogTitle><AlertDialogDescription>Thao tác này không thể hoàn tác.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Hủy</AlertDialogCancel><AlertDialogAction disabled={busy} onClick={() => submit('delete-group')}>Xóa bài đăng</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog> : null}
    {fetcher.data?.error ? <p className="basis-full text-sm text-destructive" role="alert">{fetcher.data.error}</p> : null}
  </div>;
}

function GroupedListingCard({ groupId, listing, itemLabel, canEdit, canManageHours }: { groupId: string; listing: ListingResponse; itemLabel: string; canEdit: boolean; canManageHours: boolean }) {
  return <div className="flex min-w-0 flex-col gap-3 rounded-lg border p-3"><div className="flex min-w-0 gap-3">{listing.photos[0] ? <img src={listing.photos[0]} alt="" className="size-16 shrink-0 rounded-md object-cover" /> : <div className="size-16 shrink-0 rounded-md bg-muted" />}<div className="min-w-0 flex-1"><p className="truncate font-medium">{listing.title}</p><p className="truncate text-xs text-muted-foreground">/{listing.slug}</p><div className="mt-2 flex flex-wrap gap-1">{listing.bookingModes.map((mode) => <Badge key={mode} variant="outline">{mode}</Badge>)}</div></div></div><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs text-muted-foreground">Giá từ</p><p className="text-sm font-medium">{priceFrom(listing)}</p></div><Badge variant={listing.description && listing.photos.length ? 'secondary' : 'outline'}>{listing.description && listing.photos.length ? 'Sẵn sàng' : 'Cần bổ sung'}</Badge></div><GroupedListingActions groupId={groupId} listing={listing} itemLabel={itemLabel} canEdit={canEdit} canManageHours={canManageHours} /></div>;
}

function GroupedListingActions({ groupId, listing, itemLabel, canEdit, canManageHours }: { groupId: string; listing: ListingResponse; itemLabel: string; canEdit: boolean; canManageHours: boolean }) {
  const fetcher = useFetcher<typeof action>();
  const busy = fetcher.state !== 'idle';
  return <div className="flex flex-col items-end gap-1"><div className="flex flex-wrap justify-end gap-1">{canManageHours ? <Button asChild size="xs" variant="ghost"><Link to={`/partner/listings/${listing.id}/hours`} title="Giờ hoạt động"><CalendarClock /> Giờ hoạt động</Link></Button> : null}{canEdit ? <Button asChild size="xs" variant="ghost"><Link to={`/partner/listing-groups/${groupId}/listings/${listing.id}/edit`} title={`Sửa ${itemLabel}`}><Pencil /> Sửa</Link></Button> : null}<Button size="xs" variant="ghost" disabled={!canEdit || busy} title={`Nhân bản ${itemLabel}`} onClick={() => fetcher.submit({ intent: 'duplicate-child', listingId: listing.id }, { method: 'post' })}><Copy /> Nhân bản</Button><AlertDialog><AlertDialogTrigger asChild><Button size="xs" variant="ghost" disabled={!canEdit || busy} className="text-destructive hover:text-destructive" title={`Xóa ${itemLabel}`}><Trash2 /> Xóa</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xóa {itemLabel}?</AlertDialogTitle><AlertDialogDescription>Thao tác này không thể hoàn tác.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Hủy</AlertDialogCancel><AlertDialogAction disabled={busy} onClick={() => fetcher.submit({ intent: 'delete-child', listingId: listing.id }, { method: 'post' })}>Xóa</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div>{fetcher.data?.error ? <p className="max-w-64 text-right text-xs text-destructive" role="alert">{fetcher.data.error}</p> : null}</div>;
}
