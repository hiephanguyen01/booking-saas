import type { CreateListingGroupInput, ListingGroupResponse, ListingTypeResponse } from '@booking/contracts';
...
export function ListingGroupForm({
  ...
  const fields: FieldConfig<CreateListingGroupInput>[] = [
    ...
    { name: 'photos', type: 'file', label: 'Album chung', target: 'groups', multiple: true, maxFiles: 12, colSpan: 2 },
    { name: 'address', type: 'text', label: 'Địa chỉ (tuỳ chọn)', colSpan: 1 },
          ...
          render={({ field }) => (
            <Field>
              ...
              <Input
                ...
                onBlur={field.onBlur}
                onChange={(event) => field.onChange(event.target.value.split(',').map((value) => value.trim()).filter(Boolean))}
