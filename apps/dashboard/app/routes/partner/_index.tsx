...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const [financeRes, feedRes, listingsRes, profileRes] = await Promise.all([
    ...
    canPartner(membership, 'partner.listings.read')
      ...
      : Promise.resolve(null),
    canPartner(membership, 'partner.profile.manage')
    ...
    <div className="space-y-5">
      ...
      {verificationStatus === 'rejected' ? (
        ...
        <div className="flex flex-col gap-2 rounded-lg border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning-foreground sm:flex-row sm:items-center sm:justify-between">
          <span>Hồ sơ đối tác của bạn đang chờ tenant duyệt. Bạn sẽ đăng được listing sau khi được duyệt.</span>
        ...
        <div className="flex flex-col gap-2 rounded-lg border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning-foreground sm:flex-row sm:items-center sm:justify-between">
          <span>Bạn chưa gửi thông tin định danh. Một số loại listing yêu cầu xác minh danh tính.</span>
          <Button asChild variant="outline" size="sm" className="w-fit">
            <Link to="/partner/profile" prefetch="intent">
              Gửi định danh
            </Link>
