...
export function TaxOperationsPage({
  ...
  const liability = sumTax(
    filings.filter((period) => period.status !== 'paid'),
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Thuế đối tác"
        description="Khấu trừ được ghi nhận khi dịch vụ hoàn thành; tranh chấp giữ tiền chi trả nhưng không đổi mốc ghi nhận."
        actions={
      ...
      <section className="grid gap-3 md:grid-cols-3" aria-label="Tổng quan nghĩa vụ thuế">
        ...
        <SummaryCard
          ...
          hint="Gồm hoàn thành dịch vụ và hoàn thuế đảo chiều"
        />
      </section>
...
function FilingLedger({ filings, canManage }: { filings: TaxFilingPeriodResponse[]; canManage: boolean }) {
        ...
        {canManage ? (
          <prepare.Form method="post" className="flex flex-wrap items-end gap-2">
            ...
            <div className="grid gap-1">
              <Label htmlFor="tax-month" className="text-xs">Tháng</Label>
              <NativeSelect id="tax-month" name="taxMonth" defaultValue={String(current.getMonth() + 1)}>
                {Array.from({ length: 12 }, (_, index) => index + 1).map((month) => (
                  <NativeSelectOption key={month} value={month}>Tháng {month}</NativeSelectOption>
                ))}
              </NativeSelect>
            </div>
            <div className="grid gap-1">
              <Label htmlFor="tax-year" className="text-xs">Năm</Label>
              <Input id="tax-year" name="taxYear" type="number" min={2026} max={2100} defaultValue={current.getFullYear()} className="w-28" />
            </div>
      ...
      <CardContent className="space-y-4 pt-4">
        ...
        {filings.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">
            Chưa có kỳ kê khai. Khi lập kỳ, hệ thống gom các sự kiện hoàn thành dịch vụ chưa được kê khai.
          </p>
        ) : (
          <Table>
            <TableHeader>
...
function FilingRow({ period, canManage }: { period: TaxFilingPeriodResponse; canManage: boolean }) {
  ...
  return (
    <TableRow>
      <TableCell className="font-medium tabular-nums">{String(period.taxMonth).padStart(2, '0')}/{period.taxYear}</TableCell>
      <TableCell>
        <Badge variant={STATUS_VARIANT[period.status]}>{STATUS_LABEL[period.status]}</Badge>
        {period.submissionReference ? (
          <p className="mt-1 max-w-40 truncate text-xs text-muted-foreground" title={period.submissionReference}>
            {period.submissionReference}
          </p>
        ) : null}
      </TableCell>
      ...
      <TableCell className="text-right">
        {canManage && period.status === 'draft' ? (
          <details className="inline-block text-left">
            <summary className="cursor-pointer text-sm font-medium text-primary">Nộp tờ khai</summary>
            <fetcher.Form method="post" className="mt-2 grid w-72 gap-2 rounded-md border bg-background p-3 shadow-sm">
              <input type="hidden" name="intent" value="submit" />
              <input type="hidden" name="filingId" value={period.id} />
              <Label htmlFor={`submission-${period.id}`}>Mã tiếp nhận</Label>
              <Input id={`submission-${period.id}`} name="submissionReference" required maxLength={200} placeholder="Mã hồ sơ điện tử" />
              <ActionFeedback fetcher={fetcher} />
              <Button type="submit" size="sm" disabled={busy || total < 0n}>Xác nhận đã nộp</Button>
            </fetcher.Form>
          </details>
        ) : null}
        {canManage && period.status === 'submitted' ? (
          total > 0n ? (
            <details className="inline-block text-left">
              <summary className="cursor-pointer text-sm font-medium text-primary">Ghi nhận nộp thuế</summary>
              <fetcher.Form method="post" className="mt-2 grid w-80 gap-2 rounded-md border bg-background p-3 shadow-sm">
                ...
                <Input id={`payment-ref-${period.id}`} name="paymentReference" required maxLength={200} />
                <Label htmlFor={`paid-date-${period.id}`}>Ngày nộp</Label>
                <Input id={`paid-date-${period.id}`} name="paidDate" type="date" required />
                <Label htmlFor={`evidence-${period.id}`}>Khóa tệp chứng từ (nếu có)</Label>
                <Input id={`evidence-${period.id}`} name="fileKey" maxLength={500} />
                ...
                <ActionFeedback fetcher={fetcher} />
                <Button type="submit" size="sm" disabled={busy}>Xác nhận đã nộp</Button>
              </fetcher.Form>
            </details>
          ) : (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Phát hành cho đối tác sau khi các kỳ liên quan đã nộp thuế; tệp và checksum giữ dấu vết kiểm toán.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        {canManage ? (
          <fetcher.Form method="post" className="grid gap-3 rounded-md border bg-muted/20 p-4 lg:grid-cols-6 lg:items-end">
            ...
            <div className="grid gap-1.5 lg:col-span-2">
              ...
              <NativeSelect id="certificate-partner" name="partnerId" required className="w-full">
                ...
                {partners.map((partner) => (
                  <NativeSelectOption key={partner.id} value={partner.id}>{partner.name}</NativeSelectOption>
                ))}
              </NativeSelect>
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="certificate-year">Năm</Label>
              <Input id="certificate-year" name="taxYear" type="number" min={2026} max={2100} defaultValue={currentYear} required />
            </div>
            <div className="grid gap-1.5">
            ...
            <div className="grid gap-1.5">
              <Label htmlFor="certificate-file">Khóa tệp</Label>
              <Input id="certificate-file" name="fileKey" required maxLength={500} />
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="certificate-checksum">Checksum</Label>
              <Input id="certificate-checksum" name="checksum" required minLength={16} maxLength={256} />
            </div>
            <div className="lg:col-span-6">
              <ActionFeedback fetcher={fetcher} />
            </div>
            <Button type="submit" className="lg:col-span-6 lg:w-fit" disabled={fetcher.state !== 'idle' || partners.length === 0}>
        ...
        {certificates.length === 0 ? (
          <p className="py-6 text-center text-sm text-muted-foreground">Chưa phát hành chứng từ khấu trừ nào.</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {certificates.map((certificate) => (
                <TableRow key={certificate.id}>
                  ...
                  <TableCell>{certificate.taxYear}</TableCell>
                  <TableCell className="font-mono text-xs">{certificate.certificateNumber ?? '—'}</TableCell>
                  <TableCell className="text-right tabular-nums">{formatVnd(certificate.vatAmount)}</TableCell>
                  <TableCell className="text-right tabular-nums">{formatVnd(certificate.pitAmount)}</TableCell>
