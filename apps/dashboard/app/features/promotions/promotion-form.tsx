    ...
    <Form method="post" className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Áp dụng cho" htmlFor="appliesTo">
          <Select value={scope.appliesTo} onValueChange={(v) => scope.changeAppliesTo(v as ScopeKey)}>
            <SelectTrigger id="appliesTo"><SelectValue /></SelectTrigger>
            <SelectContent>
              {scope.effectiveChoices.map((c) => (
        ...
        {scope.appliesTo !== 'all' && !scope.isSelfPartnerScope ? (
          <Field label={`Mục tiêu: ${SCOPE_LABELS[scope.appliesTo]}`} htmlFor="appliesToIdPicker">
            {scope.optionList ? (
              <Select value={scope.appliesToId} onValueChange={scope.setAppliesToId}>
                ...
                <SelectContent>
                  {scope.optionList.map((o) => (
                    <SelectItem key={o.id} value={o.id}>{o.label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ) : (
              <Input id="appliesToIdPicker" value={scope.appliesToId} onChange={(e) => scope.setAppliesToId(e.target.value)} placeholder="UUID mục tiêu" />
            )}
          </Field>
        ) : null}
...
// NOTE: deliberately kept local — features/partner/components/form-layout has a
// near-identical Field, but form-layout belongs to the partner lane and features
