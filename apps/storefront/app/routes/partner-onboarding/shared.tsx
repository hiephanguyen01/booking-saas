  ...
  return (
    <section className="flex w-full max-w-[486px] flex-col">
      <h1 className="text-[34px] font-semibold leading-[1.55] tracking-[-0.025em] text-[#161616] sm:text-[40px] sm:leading-[1.4]">
        Tăng Doanh Thu và<br />Hiệu Quả Truyền Thông
      </h1>
      <p className="mt-3 max-w-[448px] text-sm font-medium leading-6 text-[#667085]">
        Trở thành đối tác của BOOKING STUDIO khiến việc kinh doanh của bạn dễ dàng hơn bao giờ hết. Hãy cùng nhau bắt đầu nhé!
      </p>
      <img
...
export function AuthSplit({ children, tall = false }: { children: ReactNode; tall?: boolean }) {
  return (
    <main className="mx-auto grid w-full max-w-292.5 grid-cols-1 gap-10 px-5 pb-16 lg:grid-cols-[486px_566px] lg:justify-between lg:px-0 lg:pt-10">
      <div className="hidden lg:block"><PromoPanel /></div>
      <section className={`w-full self-center bg-white px-6 py-10 shadow-[0_4px_7.5px_rgba(0,0,0,0.07)] sm:px-10 ${tall ? 'min-h-[548px]' : ''}`}>
        {children}
      </section>
    </main>
  );
...
export function FormHeading({ title, description }: { title: string; description?: ReactNode }) {
  return (
    <div className="mb-10">
      ...
      {description ? <div className="mt-6 text-base font-medium leading-6 text-[#667085]">{description}</div> : null}
    </div>
  );
}
...
export function PasswordField({ name, label, error, autoFocus }: { name: string; label: string; error?: string; autoFocus?: boolean }) {
    ...
    <label className="block text-sm font-medium text-[#344054]">
      ...
      <span className="relative mt-2 block">
        <input
          ...
          placeholder="••••••••"
        />
        <button type="button" onClick={() => setVisible((value) => !value)} className="absolute right-3 top-1/2 grid size-8 -translate-y-1/2 place-items-center text-[#667085]" aria-label={visible ? 'Ẩn mật khẩu' : 'Hiện mật khẩu'}>
...
export function FormAlert({ children }: { children?: ReactNode }) {
  return children ? (
    <div role="alert" className="mb-5 flex gap-2 rounded-sm border border-[#f43f3f]/30 bg-[#fff4f4] px-4 py-3 text-sm text-[#c93434]">
      <AlertCircle className="mt-0.5 size-4 shrink-0" />{children}
    </div>
  ) : null;
}
