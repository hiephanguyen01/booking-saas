...
export async function loader({ request, params, url }: Route.LoaderArgs) {
  try {
    ...
    if (mode === 'hourly') {
      ...
      const dateValue = requestedDate && DATE_RE.test(requestedDate)
        ? requestedDate
        : todayInTz(timezone);
      ...
      const quote = selectionAvailable
        ? await fetchQuote(
            request,
            listing.slug,
            new URLSearchParams({ mode, from: start!, to: end!, quantity: '1' }),
          )
        : null;
      return {
        ok: true as const,
    ...
    const from = requestedFrom && DATE_RE.test(requestedFrom)
      ? requestedFrom
      : todayInTz(timezone);
    ...
    const range = to ? eligibleDailyRange(from, to, minNights, maxNights) : null;
    ...
    const selectionAvailable = Boolean(
      range && datesInRange(range.from, range.to).every((dateValue) => openDates.has(dateValue)),
    );
    ...
    const selectionEnd = selectionAvailable
      ...
      : null;
    const quote = selectionStart && selectionEnd
      ? await fetchQuote(
          request,
          listing.slug,
          new URLSearchParams({
