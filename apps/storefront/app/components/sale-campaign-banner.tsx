...
export function SaleCampaignRibbon({
  ...
  return (
    <span className="absolute top-6 left-0 flex h-10 w-18 items-center justify-center whitespace-nowrap pr-3 pl-1 font-['Montserrat'] text-[16px] leading-5 font-semibold text-white [background:linear-gradient(270deg,_#F43F3F_0%,_#F68C6A_100%)] [clip-path:polygon(0_0,100%_0,84%_50%,100%_100%,0_100%)]">
      <span aria-hidden="true">- {headline.percent}%</span>
      <span className="sr-only">{t('campaign.exactPercent', { percent: headline.percent })}</span>
    </span>
  );
}
