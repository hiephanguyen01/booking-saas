...
export function ReviewDialog({
  ...
  const submitting = fetcher.state !== 'idle' || media.some((item) => item.state === 'uploading');
  ...
  const addFiles = (files: File[]) => {
    setFileError(null);
    if (media.length + files.length > REVIEW_MEDIA_MAX_FILES) {
  ...
  const submit = async () => {
    ...
    for (const item of media) {
      ...
      try {
        const result = await presignAndPutReviewMedia(item.file, review.bookingId);
    ...
    <Dialog open={open} onOpenChange={(next) => !submitting && onOpenChange(next)}>
      <DialogContent className="max-h-[calc(100vh-2rem)] gap-0 overflow-hidden p-0 sm:max-w-xl">
        <DialogHeader className="border-b border-border px-5 py-5 pr-14 sm:px-6">
          ...
          <DialogDescription>
            {t('reviews.dialog.description', { title: review?.listingTitle ?? '' })}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 overflow-y-auto px-5 py-5 sm:px-6">
            ...
            >
              {[1, 2, 3, 4, 5].map((value) => {
                ...
                return (
                  ...
                  >
                    <Star
                      className={cn('size-8', active ? 'text-amber-500' : 'text-muted-foreground/35')}
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t('reviews.dialog.uploadRules')}</p>
            </div>
            <div
        ...
        <DialogFooter className="grid grid-cols-2 gap-3 border-t border-border px-5 py-4 sm:grid-cols-2 sm:px-6">
          ...
          <Button type="button" onClick={submit} disabled={!formValid || submitting}>
