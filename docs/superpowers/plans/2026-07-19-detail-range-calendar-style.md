...
**Goal:** Make the listing detail daily range calendar visually match Search and allow one-click, one-day range selection in both surfaces.

**Architecture:** Reuse the storefront's existing `sf-calendar` CSS contract rather than duplicating range styles. Remove Search's `resetOnSelect` range behavior so react-day-picker returns a complete same-day range on the first click, matching detail and the existing daily-range normalization boundary.
