...
- Group moderation writes 1 group + N children moderates + **exactly 1 audit row (group only, NOT per-child)** + 1 outbox row (group only, payload `{groupId}`) in ONE tx (run-group-moderation.ts) — corrected during PR #11c (earlier draft wrongly said "N audit rows"); no per-child audit/outbox. update-listing-group's reopen cascade fires child moderates via Promise.all inside a Prisma interactive tx — aggregate-per-child rehydration could multiply queries or hit tx timeouts on big posts
